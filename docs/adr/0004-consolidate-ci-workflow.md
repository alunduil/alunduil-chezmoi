...
The scheduled half of the repo already got this right: `weekly.yml` takes
its name from its cadence and holds the link check as job `external-links`.
...
(`systemd-analyze verify` needs a ~400 MB install, and the metrics smoke
binds real ports). Those are different whens, so they're different files.
...
- **Native per-workflow `paths:` filters.** Retained, because the
  ...
  filter-matching action. That costs a duplicated filter list, an extra
  runner per run, and a drift failure that's silent where the native
  filter's isn't.
...
A new check lands as a job in the file matching its when. Only a
genuinely different when warrants a new file, which in practice means a
