...
Superseded by [0004](0004-consolidate-ci-workflow.md).
...
path filtering as in-job conditionals. The per-workflow layout gets that
machinery for free, along with an independent status check per sensor.
...
validation drops coverage unnoticed, which is how the observability
config validation came to never run on most PRs.

## Decision

Keep one workflow per sensor. New checks land as new workflows, not as
jobs in a shared `ci.yml`. Reject consolidation rather than defer it.
...
- Workflow/justfile drift causes a check to stop running in CI
  unnoticed.
...
- The trigger boilerplate stays duplicated across files: a new sensor is
  a new file copied from an existing one. CI catches recipe/workflow
  drift rather than preventing it structurally.
- Cheap checks pay a small cost on every PR in exchange for never losing
  coverage unnoticed. Only expensive setup earns a gate.
