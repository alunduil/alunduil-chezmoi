...
Stored as `dot_config/<service>/encrypted_private_token.age` and restored (mode 600) on `chezmoi apply`. The `private_` on the file makes the token 600, while the service directory stays the conventional 755. See `dot_config/codecov/` for an existing example.
