...
- Security updates also bypass the weekly `schedule:weekly` (Defaults) automatically: `vulnerabilityAlerts` ships a built-in `schedule: []` (any time) default that the `minimumReleaseAge: "0 days"` object merges onto, not over. So no explicit schedule carve-out is needed — adding `"schedule": []` to the block is redundant. Net effect: routine PRs batch weekly, CVEs raise immediately.
