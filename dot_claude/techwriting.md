...
clichés. Below is the judgment it cannot make. Anything unstated here
defers to Google's [developer documentation style
guide](https://developers.google.com/style).

## Cut packaging

Prose passes every linter and still reads as performance. Delete:
...
- Sentences present for rhythm: the third clause balancing the first
  two, the closing restatement. Every sentence adds information.

## Sentences and paragraphs

- One idea per sentence. An `or` inside a long sentence usually marks
  a bulleted list trying to escape; a sequence of steps marks a
  numbered one.
...
- Headings name the reader's task ("Create the site"), not internal
  machinery ("Initialise the Froobus framework").
